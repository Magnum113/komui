...
- `addToCart()` и `updateCart()` управляют демо-корзиной.
...
- `assets/ozon/` - выбранные изображения карточек Ozon.
- `assets/ozon-candidates/` - кандидаты изображений, использованные при подготовке каталога.
...
7. После загрузки данных строятся фильтры, карточки каталога, бегущая строка коллекций, hero-статистика и демо-корзина.
...
- Это демо-витрина. Кнопка оформления заказа не подключена к оплате и доставке.
