...
Для худи `FIT` (`CRP`/`REG`) и `FABRIC` (`FLC`/`NF`) являются физическими
вариантами, а не техническими частями артикула. Каждая комбинация посадки и
начёса публикуется отдельной связанной карточкой. Внутри одной активной карточки
каждому нормализованному размеру должен соответствовать ровно один selectable
offer ID; архивные и `visible=false` offers в выборе не участвуют. Повтор
размера без разделения карточек запрещён проверкой БД. UUID карточки остаётся
идентификатором товара для URL/feed/ecommerce, а `offerId` определяет конкретный
физический SKU.
...
- товар добавляется в корзину с нормальным размером и точным `offerId`;
- разные посадки/наличие начёса у худи не объединены в одну карточку;
- у активной карточки нет двух selectable offers одного нормализованного
  размера.
...
- Идентичность позиции корзины включает UUID карточки и точный `offerId`.
  Checkout повторно проверяет принадлежность offer карточке и совпадение
  размера. Старые однозначные позиции без `offerId` совместимы; неоднозначные
  позиции блокируются до повторного выбора покупателем.
