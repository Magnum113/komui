...
> **Исторический Supabase-first проект.** Текущая админка должна обращаться к
> защищённым `/api/admin/...` маршрутам KOMUI backend, а не подключаться из
> браузера непосредственно к Supabase/PostgreSQL. Актуальные API-контракты:
> [`docs/admin-storefront-products-api.md`](docs/admin-storefront-products-api.md)
> и [`docs/admin-storefront-orders-api.md`](docs/admin-storefront-orders-api.md).
