...
Дата отсечения обязательна и проверяется по `created_at` shipment ещё при выборе
на синхронизацию: старые отправления не опрашиваются и не запускают новые письма.
