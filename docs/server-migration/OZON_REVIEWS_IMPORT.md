...
GET /api/v1/products/:slug/reviews?limit=20&mediaOnly=1
```
...
`mediaOnly=1` фильтрует именно список отзывов и оставляет только записи с
публичным фото или видео. Общая сводка в ответе при этом не меняется. Публичный
каталог товаров также содержит компактное поле `review_summary` (`count`,
`averageRating`, `withMedia`), чтобы карточки каталога не делали отдельный
запрос для каждого товара.
...
- каталог и страницы коллекций показывают компактные звёзды, среднюю оценку и
  количество отзывов;
- быстрый просмотр товара ведёт по ссылке к полному разделу отзывов на
  canonical-странице товара;
- на странице товара сводка находится после описания и до рекомендаций;
- сводка содержит среднюю оценку, распределение 1–5 и количество отзывов с
  медиа;
- список загружается по 8 записей с cursor-пагинацией и фильтром
  «С фото и видео»;
- фото и видео открываются в доступном native `dialog`; видео не запускается
  автоматически;
- на экране отображается нейтральная заглушка вместо аватара и только
  публичное имя `Покупатель Ozon`;
- при отсутствии отзывов, ошибке API и загрузке предусмотрены отдельные
  состояния без блокировки покупки;
- `Product` JSON-LD получает `aggregateRating`, когда у товара есть отзывы.
