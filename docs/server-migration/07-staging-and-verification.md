...
- реальное создание CDEK shipment включено только после явного разрешения
  владельца; текущий staging работает с `CDEK_CREATE_SHIPMENTS=true`;
...
8. Webhook.
9. Payment status.
10. CDEK shipment creation на staging; сейчас создаёт реальные CDEK shipment
...
- staging создаёт реальные платежи/отправления без отдельного явного
...
отдельное решение по cutover`.

Дата выполнения технической части: 27 июня 2026 года.
Дата последней актуализации: 30 июня 2026 года.
...
Свежий backup после последних backend/CDEK изменений:

```text
/var/backups/komui/daily/komui-backup-20260630T145422Z.tar.gz.gpg
```

Размер: `40 267 466 bytes`.

Проверено:

- локальный `.sha256` валиден;
- архив загружен во внешний bucket;
- companion `.sha256` загружен во внешний bucket.
...
s3://komui-backups/komui/stage/komui-backup-20260630T145422Z.tar.gz.gpg
s3://komui-backups/komui/stage/komui-backup-20260630T145422Z.tar.gz.gpg.sha256
```
...
Свежий restore drill после последних checkout/CDEK/backend изменений выполнен
30 июня 2026 года из архива:
...
- `merch_storefront_products`: `31` rows;
- `merch_customer_orders`: `13` rows;
- `merch_payment_attempts`: `13` rows;
- `merch_cdek_shipments`: `3` rows;
- временный backend на restored DB вернул `/health/ready` HTTP `200`;
- временный backend вернул `/v1/products?limit=1` HTTP `200` и `1` товар;
- длительность drill: около `3 sec`;
- временная БД удалена после проверки;
- старая временная БД `komui_restore_drill_20260627121243`, оставшаяся после
  предыдущего drill, удалена; активных `komui_restore_drill_*` БД не осталось.
...
- backend public config:
  - `runtimeMode=staging`;
  - `trafficSwitchEnabled=true`;
  - `siteUrl=https://stage.komui.ru`;
  - `tbankMode=demo`;
  - `cdekCreateShipments=true`;
...
- Ozon admin import preview/import реализован в backend, но нужна отдельная
  финальная приёмка маппинга и сценариев записи.
- Supabase dual-write для Ozon выключен флагом
  `OZON_IMPORT_WRITE_SUPABASE=false` и должен включаться только отдельным
  решением владельца.
