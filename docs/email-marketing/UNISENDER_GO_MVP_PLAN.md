...
Статус: этапы 1–6 реализованы; техническая часть этапа 7 принята на staging
...
Форма подписки в подвале сайта сейчас не подключена к backend. В рамках MVP её
следует временно убрать или явно отключить, а не показывать ложное сообщение об
успешной подписке.
...
- double opt-in и публичная форма подписки;
- редактор шаблонов;
...
| 2. База и доказуемое согласие | Миграции outbox, suppression и полей согласия | 7–8 | завершён на staging |
| 3. API-клиент и шаблон `order_paid` | Проверенный клиент и обе версии письма | 9, 11 | завершён на staging |
...
- форма подписки и double opt-in;
- собственная страница отписки;
