...
Статус: этапы 2–6 реализованы локально; развёртывание не выполнялось  
...
| --- | --- | --- | --- |
| 1. Провайдер и безопасная конфигурация | Домен, API и сервер готовы, отправка выключена | 5–6 | почти завершён; ожидает финальной DNS-проверки |
| 2. База и доказуемое согласие | Миграции outbox, suppression и полей согласия | 7–8 | реализован локально, проверен на staging-схеме с rollback |
| 3. API-клиент и шаблон `order_paid` | Проверенный клиент и обе версии письма | 9, 11 | реализован локально, реальная отправка выключена |
| 4. Worker и событие оплаты | Надёжная очередь, повторы и отсутствие дублей | 10–11 | реализован локально, worker выключен |
| 5. Webhook и suppression | Отписки, bounce и жалобы блокируют отправку | 13–14 | реализован локально, webhook выключен |
| 6. Наблюдаемость и Admin API | Healthcheck, Telegram и минимальный статус заказа | 16–17 | реализован локально, включение не выполнялось |
| 7. Staging и приёмочные тесты | Безопасная отправка только на allowlist | 18–19 | ожидает |
...
До контролируемого развёртывания этапа 5 остаётся:

1. развернуть этапы 2–5 на staging в согласованной последовательности;
2. оставить `UNISENDER_GO_WEBHOOK_ENABLED=false` до готовности callback URL;
3. включить флаг на staging и зарегистрировать webhook в Unisender с
   `event_format=json_post`, `single_event=0`, `delivery_info=0` и только
   статусами `unsubscribed`, `hard_bounced`, `spam`;
4. выполнить тестовые unsubscribe, hard bounce и replay только на staging;
5. проверить suppression и отмену ожидающего marketing-задания;
6. только после приёмки повторить регистрацию и включение на production.
...
До контролируемого развёртывания этапа 6 остаётся:

1. выполнить общий staging rollout этапов 2–6 после миграции email-схемы;
2. установить оба email worker unit, но production unit оставить выключенным;
3. включить email и worker только на staging с allowlist владельца;
4. проверить реальные состояния `pending → processing → sent`, временный
   `retry`, окончательный `failed` и очередь старше 10 минут;
5. подтвердить получение Telegram-алерта без полного email и секретов;
6. проверить `email.orderPaid` через защищённый Admin API, затем отдельно
   вывести эти поля в интерфейсе GetoMerch;
7. оставить production email-флаги выключенными до завершения этапа 7.
